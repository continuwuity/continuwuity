...
impl Service {
	...
	/// Compresses the input state event stream
	...
	/// Compresses a single state event by creating a short event ID for it.
	...
	/// Gets the diff uin by the shortstatehash. The diff is a set of added and
	/// removed
	...
	/// Stores a new state diff with a shortstatehash.
...
/// Compresses a single state event.
#[must_use]
...
/// Parses a compressed state event identifier into a (ssh, short eventid)
/// tuple.
#[must_use]
...
/// Gets the size of a compressed state group.
