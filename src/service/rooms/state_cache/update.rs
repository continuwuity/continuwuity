...
impl super::Service {
	...
	/// Updates membership counts for the target room.
	...
	///
	/// `leave_pdu` represents the m.room.member event which the user sent to
	/// leave the room. If this is None, no event was actually sent, but we
	/// must still behave as if the user is no longer in the room. This may
	/// occur, for example, if the room being left has been server-banned by an
	/// administrator.
	...
	/// Immutably a user as having joined a room.
	fn mark_as_once_joined(&self, user_id: &UserId, room_id: &RoomId) {
		let key = (user_id, room_id);
		self.db.roomuseroncejoinedids.put_raw(key, []);
	}

	/// Marks a user as having been invited to a room, saving the relevant
	/// invite state.
