...
impl super::Service {
	/// Handles a batched notary request.
	/// If the batch is empty, an empty vec is returned.
	...
	/// Makes a request to a notary server to fetch signing keys.
	...
	/// Makes a request to an origin server to fetch signing keys.
