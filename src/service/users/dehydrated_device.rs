...
impl super::Service {
	...
	///
	/// Calling this directly will remove the dehydrated data but leak the
	/// frontage device. Thus this is called by the regular device interface
	/// such that the dehydrated data will not leak instead.
	///
	/// If device_id is given, the user's dehydrated device must match or this
	/// is a no-op, but an Err is still returned to indicate that. Otherwise
	/// returns the removed dehydrated device_id.
