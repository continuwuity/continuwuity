...
const PUSH_GATEWAY_SCHEMES: &[&str] = &["http", "https"];
const WEBPUSH_SCHEMES: &[&str] = &["https"];

/// Whether the pusher opted out of badge counts (MSC4076).
pub(crate) fn badge_counts_disabled(data: &JsonObject) -> bool {
	data.contains_key("org.matrix.msc4076.disable_badge_count")
		|| data.contains_key("disable_badge_count")
}
...
impl crate::Service for Service {
	fn build(args: crate::Args<'_>) -> Result<Arc<Self>> {
		let vapid = Vapid::init(args.db, args.server.config.webpush_vapid_contact().to_string())?;
...
impl Service {
	pub async fn set_pusher(
		...
		match &pusher.kind {
			| PusherKind::Http(http) => {
				self.validate_push_url(&http.url, PUSH_GATEWAY_SCHEMES)?;
			},
			| PusherKind::WebPush(data) => {
				self.validate_push_url(&data.url, WEBPUSH_SCHEMES)?;
		...
		if !needs_activation && pusher.kind.as_webpush().is_none() {
		...
		// Must run after the put: it delivers straight away, and a rejection
		// from the push service deletes the pusher it is validating.
		if needs_activation {
	...
	/// address.
	fn validate_push_url(&self, url: &str, allowed: &[&str]) -> Result<url::Url> {
			...
			+ Send,
	{
		...
		if !status.is_success() {
			debug_warn!("Push gateway response body: {:?}", string_from_bytes(response.body()));
	...
	async fn execute_pusher_request(
		...
		let mut result = http::Response::new(Vec::new());
		*result.status_mut() = response.status();
		*result.version_mut() = response.version();
		mem::swap(response.headers_mut(), result.headers_mut());
