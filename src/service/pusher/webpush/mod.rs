...
use self::helpers::{build_payload, constant_time_eq};
use super::{Service, WEBPUSH_SCHEMES};
...
const RETRY_MAX: Duration = Duration::from_secs(20);
...
enum Attempt {
	...
	/// The subscription is gone; delete the pusher.
	Rejected,

	/// Transient, carrying the delay the push service asked for.
...
impl Service {
	pub fn webpush_vapid_public_key(&self) -> &str { self.vapid.public_key() }
	...
	pub(super) fn begin_webpush_activation(&self, sender: &UserId, pusher: &Pusher) {
		// Send a one-time encrypted token before allowing notification
		// delivery.
	...
	pub async fn activate_webpush_pusher(
		...
		for pushkey in candidates {
			...
			if !state
				.ack_token
				.as_deref()
				.is_some_and(|token| constant_time_eq(token, ack_token))
			{
				continue;
			}
	...
	pub(super) async fn send_webpush_notice(
		...
		let prio = if data.format.as_ref() == Some(&PushFormat::EventIdOnly) {
		...
		self.deliver_webpush(user, pusher, data, payload, prio)
			.await
	}

	#[tracing::instrument(skip(self, pusher, data, payload), level = "debug")]
	/// Encrypts and sends a Web Push request, retrying transient failures and
	/// deleting rejected subscriptions.
	async fn deliver_webpush(
		...
		let endpoint = self.validate_push_url(&data.url, WEBPUSH_SCHEMES)?;
		let authorization = self.vapid.authorization(&endpoint)?;
		let pushkey = pusher.ids.pushkey.as_str();

		let mut attempt = 1_u32;
		loop {
			match self
				.attempt_webpush(user, pusher, data, &payload, &authorization, &prio)
				.await?
			{
				| Attempt::Delivered => {
				...
				| Attempt::Failed(retry_after) => {
					if attempt >= DELIVERY_ATTEMPTS {
						...
						return Err!(BadServerResponse(warn!(
							%user, %pushkey,
							"Push service did not accept the notification after {attempt} attempts"
						)));
					}

					let delay = retry_after
						.unwrap_or_else(|| RETRY_BASE.saturating_mul(attempt))
						.min(RETRY_MAX);
					sleep(delay).await;
	...
	/// The message is re-encrypted per attempt, since each needs its own
	/// ephemeral key.
	async fn attempt_webpush(
		&self,
		user: &UserId,
		pusher: &Pusher,
		data: &WebPushPusherData,
		...
		headers.insert(
			"urgency",
			if *prio == NotificationPriority::High {
				"high"
			} else {
				"normal"
			}
			...
			.await
		{
			...
			| Err(e) => {
				debug_warn!(%user, %pushkey, "Could not reach the push service: {e}");

				return Ok(Attempt::Failed(None));
			},
		};
		...
		debug_warn!(%user, %pushkey, "Push service returned unsuccessful HTTP response: {status}");
	...
	async fn back_off_webpush(
		...
		let delay = retry_after.map_or_else(
			...
			|retry_after| retry_after.min(BACKOFF_MAX),
		);
