...
use crate::pusher::badge_counts_disabled;
...
pub(super) fn build_payload(notify: &Notification, data: &JsonObject) -> Result<Vec<u8>> {
	...
	} else if !badge_counts_disabled(data) {
		payload.insert("unread".to_owned(), 0.into());
	}
...
mod tests {
	...
	use super::{MAX_BODY_LENGTH, build_payload, constant_time_eq};
	...
	#[test]
	fn keeps_zero_unread_count() {
		let payload = payload_of(&Notification::new(vec![]), &JsonObject::new());

		assert_eq!(payload.get("unread"), Some(&json!(0)));
	}

	#[test]
	fn omits_count_when_badges_are_disabled() {
		let mut data = JsonObject::new();
		data.insert("org.matrix.msc4076.disable_badge_count".to_owned(), json!(true));
		let payload = payload_of(&Notification::new(vec![]), &data);

		assert!(!payload.contains_key("unread"));
	}
