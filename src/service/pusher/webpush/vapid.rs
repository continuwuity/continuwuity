...
pub(crate) struct Vapid {
	...
	public_key: String,
	contact: String,
	authorizations: SyncRwLock<HashMap<String, (String, u64)>>,
}

impl Vapid {
	/// Loads the persistent VAPID keypair, generating one on first use.
	pub(crate) fn init(db: &Arc<Database>, contact: String) -> Result<Self> {
	...
	pub(super) fn authorization(&self, endpoint: &url::Url) -> Result<String> {
		let audience = endpoint.origin().ascii_serialization();
		let now = utils::millis_since_unix_epoch().saturating_div(1000);

		if let Some((header, expires_at)) = self.authorizations.read().get(&audience)
		...
		let header = self.sign(&audience, expires_at)?;
		let mut authorizations = self.authorizations.write();
		authorizations.retain(|_, (_, expires_at)| now < *expires_at);
		authorizations.insert(audience, (header.clone(), expires_at));

		Ok(header)
	}

	fn sign(&self, audience: &str, expires_at: u64) -> Result<String> {
		let header =
