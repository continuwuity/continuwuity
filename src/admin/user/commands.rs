...
impl crate::Context<'_> {
	...
	pub(super) async fn reset_password(
		...
		if user_id == self.services.globals.server_user {
			return Err!(
				"Not allowed to set the password for the server account. Please use the \
				 emergency password config option.",
			);
		}
		...
		for user_id in user_ids {
			match self.services.users.deactivate_account(&user_id).await {
				...
				| Ok(()) => {
					...
					if !no_leave_rooms {
						info!(
							"Forcing user {user_id} to leave all rooms apart of deactivate-all"
						);
						let all_joined_rooms: Vec<OwnedRoomId> = self
							.services
							.rooms
	...
	pub(super) async fn force_join_list_of_local_users(
		...
		if !yes_i_want_to_do_this {
			return Err!(
				"You must pass the --yes-i-want-to-do-this-flag to ensure you really want to \
				 force bulk join all specified local users.",
			);
		}
		...
		for user_id in user_ids {
				...
				.await
			{
				...
				| Err(e) => {
					debug_warn!(
						"Failed force joining {user_id} to {room_id} during bulk join: {e}"
					);
	...
	pub(super) async fn force_join_all_local_users(
		...
		if !yes_i_want_to_do_this {
			return Err!(
				"You must pass the --yes-i-want-to-do-this-flag to ensure you really want to \
				 force bulk join all local users.",
			);
		}
	...
	pub(super) async fn put_room_tag(
		...
		self.write_str(&format!(
			"Successfully updated room account data for {user_id} and room {room_id} with tag \
			 {tag}"
		))
		.await
	}
	...
	pub(super) async fn delete_room_tag(
		...
		self.write_str(&format!(
			"Successfully updated room account data for {user_id} and room {room_id}, deleting \
			 room tag {tag}"
		))
		.await
	}
		...
		match (current_email, new_email) {
			...
			| (current_email, Some(new_email)) => {
				...
				if let Some(current_email) = current_email {
					self.write_str(&format!(
						"The associated email of {user_id} has been changed from \
			...
			| (Some(current_email), None) => {
				...
				self.write_str(&format!(
					"The associated email of {user_id} has been removed (it was \
					 {current_email})."
				))
				.await
			},
